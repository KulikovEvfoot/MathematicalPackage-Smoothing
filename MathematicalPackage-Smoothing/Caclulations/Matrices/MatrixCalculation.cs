...
namespace MathematicalPackage_Smoothing.Caclulations.Matrices
{
    ...
    {
        protected float InitLambdaType2_0(float h1, float h2, float a, float p1, float p2, float p3)
        {
        ...
        protected float InitFirstMuType2_0(float firstH, float a, float firstP, float secondP, float secondH)
        {
            return  (firstH / 6) - a * ((firstP / (float)Math.Pow(firstH, 2)) + 
                                       ((secondP / firstH) * ((1 / firstH) + (1 / secondH))));
        }

        protected float InitLastMuType2_0(float h1, float a, float p1, float p2, float h2)
        {
            return (h1 / 6) - a * ((p1 / (float)Math.Pow(h1, 2)) +
                                  ((p2 / h1) * ((1 / h2) + (1 / h1))));
        }

        protected float InitMuType2_0(float h1, float a, float p1, float h2, float p2, float h3)
        {
            return (h1 / 6) - (a / h1) * (p1 * ((1 / h2) + (1 / h1)) + p2 * ((1 / h1) + (1 / h3)));
        }

        protected float InitBetaType2_0(float a, float p1, float h1, float h2)
        {
            return a * (p1 / (h1 * h2));
        }
        ...
        {
            return InitFirstLambdaType1(h1, a, p1, p2);
        }

        protected float InitLambdaType1(float h1, float h2, float a, float p1, float p2, float p3)
        {
            return InitLambdaType2_0(h1, h2, a, p1, p2, p3);
        }
